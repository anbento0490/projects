...
- `NAV-Proportional Scaling (Output 2)`: this method instead scales the `company valuations` so that they sum exactly to the fund NAV on that date. It adjusts for any differences by recalculating the percentage on a daily basis. This means that if the company valuations deviate from what the cumulative commitments would imply, the scaling factor will automatically adjust.

**Reason For Discrepancies:** the second method **only processes** company valuation records for which there is a corresponding fund NAV on the same fund and date. In other words, output 2 is generated by joining the companies transactions with the fund NAV data, and if a company valuation record doesn't have a matching fund NAV (i.e. the fund_nav table doesn't contain an entry for that specific date), that record is excluded from the final result -> Output 2 includes 59x records while Output 2 includes the original 61 records.
